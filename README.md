...
- 네이버 지도 api로 출발지와 도착지 거리 및 시간을 받아옴 --> 네이버 정책상 경로 관련 api는 ip주소로 호환하기 때문에, proxy 서버를 cdn(render)에 배포하여 데이터를 받음 --> 
이슈) (1) 네이버 api로 이동 경로를 받아오기가 어려움 (2) 네이버 api로는 도보 이동 경로를 받아올 수가 없음 --> 해결) T map api로 도보 이동 거리 및 자동차 이동 거리를 데이터로 받아옴 
