...
> `Next.js`, `recoil`, , `Naver Maps API`, `T Maps API` 등 다양한 기술을 통합한 **실전형 포트폴리오 프로젝트**입니다.
...
**[이슈]**
1. 로컬 서버에서는 시각화된 데이터가 잘 보이는데,  
   Vercel 배포 환경에서는 데이터가 시각화되지 않음


**[원인]**
1. 기존에는 **Firebase Web SDK**를 사용했음  
   - Web SDK는 **클라이언트 전용**으로 브라우저에서만 동작  
   - 따라서 **서버 컴포넌트**에서는 사용할 수 없음


**[해결]**
1. **Admin SDK(firebase-admin) 사용**
   - Admin SDK는 인증/보안 규칙을 무시하고 **서버 자격 증명**으로 접근 가능
   - 하지만 Admin SDK가 클라이언트로 노출되면 보안상 문제 발생
   - 안전 장치로 해당 모듈 파일 최상단에 작성:
     ```ts
     import 'server-only'
     ```
...
2. **Next.js 플래그 지정**
   - App Router는 기본적으로
     - **정적 최적화**: 빌드 시점에 HTML을 생성 (SSG/ISR/PPR 포함)
     - **Edge Runtime**: 사용자와 가까운 곳에서 코드 실행해 네트워크 왕복 최소화
   - 하지만 Admin SDK는 **Node 전용**이므로 Edge 불가
   - 또한 항상 최신 DB 읽기가 필요하므로 정적 최적화도 부적합
   - 따라서 아래 설정을 추가:
     ```ts
     export const runtime = 'nodejs'        // Edge가 아닌 Node.js 런타임에서 실행 강제
     export const dynamic = 'force-dynamic' // 정적 빌드/캐싱 금지 → 매 요청마다 서버 실행
     export const revalidate = 0            // ISR/캐싱 금지 → 항상 최신 데이터 읽기
     ```

**[SSR을 선택한 이유]**
1. **백엔드 서버를 별도로 두지 않고**, Firestore에서 데이터를 직접 가져오기 위해
2. 추후 계정마다 많은 기록을 불러와야 할 때,  
   브라우저에서 직접 렌더링하는 것보다 **서버에서 미리 렌더링**하는 것이 더 적합하다고 판단
3. 단순히 **리로드로 데이터를 읽고 삭제**하는 기능만 필요하기 때문에,  
   **실시간 구독(CSR 하이브리드)**까지는 불필요
